...
export default class Player extends Component {
  ...
  randomSongIndex(){
    let maxIndex = Artists[clippingsStore.artistIndex].songs.length - 1;
  ...
  render() {
    let songPlaying = Artists[clippingsStore.artistIndex].songs[this.state.songIndex];
    let songPercentage;
    ...
    let forwardButton;
    if( !this.state.shuffle && this.state.songIndex + 1 === Artists[clippingsStore.artistIndex].songs.length ){
      forwardButton = <Icon style={ styles.forward } name="ios-skipforward" size={25} color="#333" />;
    } else {
      forwardButton = <Icon onPress={ this.goForward.bind(this) } style={ styles.forward } name="ios-add" size={25} color="#fff" />;
    }
    ...
    let image = songPlaying.albumImage ? songPlaying.albumImage : Artists[clippingsStore.artistIndex].background;
    return (
      <View style={styles.container}>
        ...
        <View style={ styles.header }>
          <Text style={ styles.headerText }>
            { Artists[clippingsStore.artistIndex].name }
          </Text>
        </View>

        <Image style={ [styles.songImage, {width: window.width - 30, height: 300} ]} source={{uri: image}}/>
